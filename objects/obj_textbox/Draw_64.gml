...
if(cname != "None"){
    ...
    draw_text_color(name_box_text_x, name_box_text_y, string_hash_to_newline(cname), c,c,c,c, 1);
...
if(type[page] == 1){
    ...
    repeat(tpl){
        ...
        //Draw our choices
        var ctext = "* "+tp[ii];
        draw_text_ext_color(xx, yy+((ii+iy)*stringHeight), string_hash_to_newline(ctext), stringHeight, txtwidth, col, col, col, col, 1);
       
        if(string_width(string_hash_to_newline(ctext)) > txtwidth) { iy++; }
        ii++;
    }
}
...
//---------TYPE 0: NORMAL DIALOGUE
    ...
    repeat(charCount){
        ...
        switch(effect){
            case 0: //normal
                draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight), string_hash_to_newline(letter), col, col, col, col, 1);
                break;
           
            case 1: //shakey
                draw_text_color(xx + (cx*charSize)+random_range(-1,1), yy+(cy*stringHeight)+random_range(-1,1), string_hash_to_newline(letter), col, col, col, col, 1);
                break;
           
            case 2: //wave
                var so = t;
                var shift = sin(so*pi*freq/room_speed)*amplitude;
                draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight)+shift, string_hash_to_newline(letter), col, col, col, col, 1);
                break;
           
            case 3: //colour shift
                var c1 = make_colour_hsv(t+cc, 255, 255);
                var c2 = make_colour_hsv(t+cc+34, 255, 255);
                draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight), string_hash_to_newline(letter), c1, c1, c2, c2, 1);
                break;
            ...
            case 4: //wave AND colour shift
                ...
                draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight)+shift, string_hash_to_newline(letter), c1, c1, c2, c2, 1);
                break;
            ...
            case 5: //spin
                ...
                draw_text_transformed_color(xx + (cx*charSize)+mv, yy+(cy*stringHeight)+(stringHeight/2), string_hash_to_newline(letter), 1, 1, shift*20, col, col, col, col, 1);
            ...
            case 6: //pulse
                ...
                var mv = charSize/2;
                draw_set_valign(fa_middle); draw_set_halign(fa_middle);
                draw_text_transformed_color(xx + (cx*charSize)+mv, yy+(cy*stringHeight)+(stringHeight/2), string_hash_to_newline(letter), shift, shift, 0, col, col, col, col, 1);
                draw_set_valign(fa_top); draw_set_halign(fa_left);
                break;
            ...
            case 7: //flicker
                var so = t + cc;
                var shift = sin(so*pi*freq/room_speed);
                draw_text_color(xx + (cx*charSize), yy+(cy*stringHeight), string_hash_to_newline(letter), col, col, col, col, shift+random_range(-1,1));
                break;
        }
