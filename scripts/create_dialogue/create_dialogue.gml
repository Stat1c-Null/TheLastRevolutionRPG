/// @description create_dialogue(myText, mySpeaker, myEffects, myTextSpeed, myTypes, myNextLine, myScripts, myTextCol, myEmotion, myEmote);
/// @param myText
/// @param  mySpeaker
/// @param  myEffects
/// @param  myTextSpeed
...
//Create the Textbox
var _textbox = instance_create(x,y, obj_textbox);
...
if(!is_array(arg[1]))   { _speaker = array_create_fill(text_len, id); }
else                    { _speaker = arg[1]; }

//Get rest of arguments, fill with default
var _effects    = array_create_fill(text_len, create_array(1,0));
var _speed      = array_create_fill(text_len, create_array(1,0.5));
var _textcol    = array_create_fill(text_len, create_array(1,c_white));
var _type       = array_create_fill(text_len, 0);
var _nextline   = array_create_fill(text_len, 0);
var _script     = array_create_fill(text_len, 0);
var _emotion    = array_create_fill(text_len, 0);
var _emotes     = array_create_fill(text_len, -1);
var _creator    = array_create_fill(text_len, id);
...
with(_textbox){
    ...
    i = 0; repeat(text_len){
        ...
        if(portrait_talk[i] != -1){
            ...
            portrait_talk_s[i] = _speaker[i].myPortraitTalk_imgspd;
        }
        if(portrait_idle[i] != -1){
            portrait_idle_n[i] = sprite_get_number(portrait_idle[i]);
            portrait_idle_s[i] = _speaker[i].myPortraitIdle_imgspd;
        }
        i++;
    }
   
    draw_set_font(font[0]);
    charSize = string_width(string_hash_to_newline("M"));
    stringHeight = string_height(string_hash_to_newline("M"));
